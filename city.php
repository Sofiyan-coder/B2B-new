...
<br>
  <div class="select-box">
    <select name="taluka_id" id="taluka_Select">
      <option value="">--Select Taluka--</option>
      <!-- Districts will be populated dynamically using JavaScript -->
    </select>
  </div>

  <span>Enter city Name:</span><br>
   <input type="text" name="city_name"><br><br>
 
  <button type="submit" name="add_city" value="add_city">Add City</button>
  <br><br>
...
const taluka_Select= document.getElementById('taluka_Select');
...
district_Select.addEventListener('change', () => {
  const selectedDistrictId = district_Select.value;
...
function fetchTalukas(districtId) {
  taluka_Select.innerHTML = '<option value="">--Select Taluka--</option>';
    ...
    fetch(`get_talukas.php?district_id=${districtId}`)
      ...
      .then(talukas => {
        talukas.forEach(taluka => {
          ...
          taluka_Select.appendChild(option);
        });
      })
      .catch(error => {
        console.error('Error fetching states:', error);
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (isset($_POST['add_city'])) {
    $taluka_id = $_POST['taluka_id']; // Fix the attribute name here
    $city_name = $_POST['city_name'];

    $sql = "INSERT INTO city ( C_nm, T_id) VALUES ( '$city_name', '$taluka_id')";
